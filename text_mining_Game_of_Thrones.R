...
#如何用R对《权利的游戏》故事情节做情绪分析？

#情绪分析(emotional analysis),情感分析(sentiment analysis)
#情感分析的结果一般分为正向(positive)和负向(negative)，而情绪分析包含的种类就比较多
#加拿大国家研究委员会(National Research Council of Canada)官方发布的情绪词典包含了8种：
#1.愤怒(anger)
#2.期待(anticipation)
#3.厌恶(disgust)
#4.恐惧(fear)
...
#准备
#数据：《权利的游戏》第三季第9集，"The Rains of Castamere"
...
#如果以1行为单位分析情感变化，粒度过细
#鉴于整个剧本包含了几百行文字，以5行作为一个基础单位，进行分析

#使用index把原先的行号处理一下，分成段落
# %/% 代表整除符号，这样0-4行就成了第一段落，5-9行成为第二段落，以此类推
...
#疑惑：按道理来说，每一段落的内容里，包含单词数量大致相当。
#结尾部分情感分析结果里面，positive和negative几乎同时上升，这就让人很不解
#是这里的几行太长了，还是出了什么其他的问题呢？
...
#处理停用词，对于每一个具体场景，都需要使用停用词表，把那些可能
#干扰分析结果的词扔出去
